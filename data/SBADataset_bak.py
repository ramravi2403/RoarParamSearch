...
class SBADataset(Dataset):
    ...
    def get_data(
        ...
        if introduce_leakage:
            ...
            self.scaler.fit(X_combined[cont_cols])
        else:
            self.scaler.fit(X_train[cont_cols])

        X_train = self._scale_dataframe(X_train, cont_cols)
        X_test = self._scale_dataframe(X_test, cont_cols)
        X_query = self._scale_dataframe(X_query, cont_cols)
    ...
    def _process_full_features(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, List[str], List[str]]:
        ...
        # Identify continuous columns after OHE
        cont_cols = [
            col for col in df.columns
            if col != self.TARGET
               and df[col].dtype in ['float32', 'float64', 'int64']
               and len(df[col].unique()) > 2
        ]

        feature_names = [col for col in df.columns if col != self.TARGET]

        return df, feature_names, cont_cols
    ...
    def _scale_dataframe(self, X: pd.DataFrame, cont_cols: List[str]) -> pd.DataFrame:
        ...
        if len(cont_cols) == 0:
            return X
        X_scaled = X.copy()
        X_scaled[cont_cols] = self.scaler.transform(X[cont_cols])
        return X_scaled
