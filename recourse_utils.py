...
def lime_explanation(model_pred_proba, X_train, x):
	# 1. EXPLALINER INITIALIZATION (Fixes NameError and initial ValueError)
	explainer = lime.lime_tabular.LimeTabularExplainer(
		training_data=X_train,
		class_names=['0', '1'],  # CRITICAL: Ensures LIME knows it's classification
		mode='classification',  # Forces classification mode
		discretize_continuous=False,
		feature_selection='none'
	)

	# 2. EXPLAIN INSTANCE (Fixes error from explicit model_regressor)
	exp = explainer.explain_instance(x,
									 model_pred_proba,
									 num_features=X_train.shape[1])
	# model_regressor=LogisticRegression()) <-- Removed this line

	# 3. COEFFICIENT EXTRACTION (Fixes RuntimeError: inconsistent tensor size)

	# Get the number of features (28 for SBA)
	num_features = X_train.shape[1]

	# Initialize the coefficient vector with zeros
	coefficients_vector = np.zeros(num_features)

	# Extract the list of (feature_index, coefficient) tuples for class 1
	# local_exp[1] refers to the explanation for class 1 (the target class)
	coef_tuples = exp.local_exp[1]

	# Populate the vector using the indices and values from the LIME output
	for feat_idx, coef_val in coef_tuples:
		# LIME indices correspond to the column index in X_train
		coefficients_vector[feat_idx] = coef_val

	# The intercept is the single value for class 1
	intercept = exp.intercept[1]

	# Return the full 28-element vector and the 1-element intercept
	return coefficients_vector, intercept
def lime_explanation2(model_pred_proba, X_train, x):
