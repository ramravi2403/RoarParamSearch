...
class RobustRecourse():
    ...
    def set_W0_lime(self, W0):
        if W0 is not None:
            W0_array = np.atleast_1d(W0).astype(np.float32)
            self.W0 = torch.from_numpy(W0_array).float()
    ...
    def set_pW0(self, pW0):
        self.pW0 = pW0
        if pW0 is not None:
            # FIX 7: Convert to float32 before PyTorch conversion
            self.pW0 = torch.from_numpy(pW0.astype(np.float32)).float()
    ...
    def calc_delta_opt_lime(self, recourse):
        """
        ...
        W = torch.cat((W_safe, W0_safe), 0)  # Add intercept to weights
        ...
        c = list(np.array(gradient_w_loss.detach().cpu()) * np.array([-1] * len(gradient_w_loss)))
    ...
    def get_recourse(self, x, lamb=0.1,norm = 1,lime=False):
        ...
        x_new = Variable(x.clone(), requires_grad=True)
        optimizer = optim.Adam([x_new],lr = 0.5)
...
class CausalRecourse():
    def __init__(self, X_train, predict_proba_fn, torch_model,
        ...
        self.feature_costs = feature_costs
        if self.feature_costs is not None:
            # FIX 9: Convert to float32 before PyTorch conversion
            self.feature_costs = torch.from_numpy(feature_costs.astype(np.float32)).float()
    ...
    def get_grad(self, x, rec, lime_seed):
        # FIX 10: Convert to float32 before PyTorch conversion
        x = torch.from_numpy(x.astype(np.float32)).float()
        # FIX 11: Convert to float32 before PyTorch conversion
        rec = Variable(torch.from_numpy(rec.astype(np.float32)).float(), requires_grad=True)
