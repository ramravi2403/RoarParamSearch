...
class CombinedEvaluator:
    ...
    def run_single_combination(
        ...
        extraction_metrics = evaluate_extraction(
            ...
            baseline_model, feature_names, alpha,
            X_train=X_train,
            y_train=y_train,
    ...
    def print_summary(self):
        ...
        # 7. Impact of Query Set Size (Updated column names)
        self.__print("\n" + "=" * 70)
    ...
    def print_summary2(self):
        ...
        best_quality = df.loc[df['quality_score'].idxmax()]
        self.__print("\n🏆 Best Overall Quality Score:")
        self.__print(f"  δ_max={best_quality['delta_max']:.3f}, λ={best_quality['lamb']:.3f}, α={best_quality['alpha']:.5f}")
        self.__print(f"  Quality: {best_quality['quality_score']:.4f}")
        self.__print(f"  CF Distance: {best_quality['cf_mean_distance']:.4f}, Extracted Acc: {best_quality['extracted_accuracy']:.4f}")


        best_l1 = df.loc[df['cf_mean_distance'].idxmin()]
        self.__print("\n📏 Lowest CF L1 Distance:")
        self.__print(f"  δ_max={best_l1['delta_max']:.3f}, λ={best_l1['lamb']:.3f}, α={best_l1['alpha']:.5f}")
        self.__print(f"  CF Distance: {best_l1['cf_mean_distance']:.4f}")
        self.__print(f"  Extracted Acc: {best_l1['extracted_accuracy']:.4f}, Quality: {best_l1['quality_score']:.4f}")

        best_acc = df.loc[df['extracted_accuracy'].idxmax()]
        self.__print("\n🎯 Highest Extraction Accuracy:")
        self.__print(f"  δ_max={best_acc['delta_max']:.3f}, λ={best_acc['lamb']:.3f}, α={best_acc['alpha']:.5f}")
        self.__print(f"  Extracted Acc: {best_acc['extracted_accuracy']:.4f}")
        self.__print(f"  CF Distance: {best_acc['cf_mean_distance']:.4f}, Quality: {best_acc['quality_score']:.4f}")

        best_agree = df.loc[df['extracted_agreement'].idxmax()]
        self.__print("\n🤝 Highest Agreement with Baseline:")
        self.__print(f"  δ_max={best_agree['delta_max']:.3f}, λ={best_agree['lamb']:.3f}, α={best_agree['alpha']:.5f}")
        self.__print(f"  Agreement: {best_agree['extracted_agreement']:.4f}")
        self.__print(f"  CF Distance: {best_agree['cf_mean_distance']:.4f}, Quality: {best_agree['quality_score']:.4f}")
        ...
        for delta in sorted(df['delta_max'].unique()):
            subset = df[df['delta_max'] == delta]
            self.__print(f"\nδ_max = {delta:.3f}:")
            self.__print(f"  Mean CF L1: {subset['cf_mean_l1'].mean():.4f} ± {subset['cf_mean_l1'].std():.4f}")
            self.__print(f"  Mean Extracted Acc: {subset['extracted_accuracy'].mean():.4f}")
            self.__print(f"  Mean Quality: {subset['quality_score'].mean():.4f}")
        ...
        for size_pct in sorted(df['query_size_pct'].unique()):
            ...
            self.__print(f"\nQuery Size = {size_pct * 100:.0f}% (avg {n_cfs_avg:.0f} CFs, {n_ccfs_avg:.0f} CCFs):")
            self.__print(f"  Mean CF L1: {subset['cf_mean_l1'].mean():.4f} ± {subset['cf_mean_l1'].std():.4f}")
            self.__print(
                f"  Mean Extracted Acc: {subset['extracted_accuracy'].mean():.4f} ± {subset['extracted_accuracy'].std():.4f}")
            self.__print(f"  Mean Agreement: {subset['extracted_agreement'].mean():.4f}")
            self.__print(f"  Mean Quality: {subset['quality_score'].mean():.4f}")
            self.__print(f"  CF Success Rate: {subset['cf_success_rate'].mean():.2%}")
